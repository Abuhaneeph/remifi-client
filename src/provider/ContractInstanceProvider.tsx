...
// Thirdweb RPC endpoint (aligned with target chain)
const THIRDWEB_RPC_URL = `https://${CURTIS_CHAIN_ID}.rpc.thirdweb.com/${import.meta.env.VITE_THIRDWEB_CLIENT_ID}`;
...
export const ContractInstanceProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const isCorrectNetwork = activeChain?.id === CURTIS_CHAIN_ID;
        ...
        try {
          ...
          if (wallet.getChain && wallet.switchChain) {
            try {
              ...
              if (!currentChain || currentChain.id !== CURTIS_CHAIN_ID) {
          ...
          if (typeof window !== 'undefined' && window.ethereum) {
            ...
            if (Number(network.chainId) === CURTIS_CHAIN_ID) {
              setSigner(signer);
              setConnectionError(null);
              console.log('✅ EIP-1193 signer created successfully');
              return;
            } else {
              console.warn('Wrong network detected:', network.chainId, 'expected:', CURTIS_CHAIN_ID);
              setNetworkError(`Wrong network. Expected chain ID: ${CURTIS_CHAIN_ID}, got: ${network.chainId}`);
            }
          }
  ...
  useEffect(() => {
    if (isConnected && activeChain?.id && activeChain.id !== CURTIS_CHAIN_ID) {
      setNetworkError(`Connected to wrong network. Please switch to chain ID: ${CURTIS_CHAIN_ID}.`);
  ...
  const reconnectSigner = async (): Promise<void> => {
    try {
      ...
      if (!isCorrectNetwork) {
        throw new Error(`Wrong network. Please switch to chain ID: ${CURTIS_CHAIN_ID}`);
      }
